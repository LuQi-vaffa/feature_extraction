...
all_flows_data_pcap1 = rdpcap(r"E:\pycharm project\feature extracton\doubai.pcap")
all_flows_data_pcap2 = rdpcap(r"E:\pycharm project\feature extracton\jingdong.pcap")

# 提取每个数据包的五元组，返回结果是一个列表
def dealdata(all_flows_data_pacp):
    all_flows_data = all_flows_data_pacp
    all_five_tuple = []
...
def extractflow1():
    flow = {}
    all_five_tuple = dealdata(all_flows_data_pcap1)
    all_pcap_data = rdpcap(r"E:\pycharm project\feature extracton\doubai.pcap")
...
def extractflow2():
    flow = {}
    all_five_tuple = dealdata(all_flows_data_pcap2)
    all_pcap_data = rdpcap(r"E:\pycharm project\feature extracton\jingdong.pcap")
    for i in range(len(all_five_tuple)):
        if all_five_tuple[i] not in flow.keys():
            flow[all_five_tuple[i]] = []
    for i in range(len(all_five_tuple)):
...
def extracinformation_3(fun):
    ...
    return max_win_adv, min_win_adv, zero_win_adv, avg_win_adv, flowlen



def wcsv1():
    pro, srcip, dstip, sport, dport = extracinformation(extractflow1)
    min_data_ip, q1_data_ip, med_data_ip, mean_data_ip, q3_data_ip, max_data_ip, var_data_ip, min_data_wire, q1_data_wire, med_data_wire, mean_data_wire, q3_data_wire, max_data_wire, var_data_wire = extracinformation_1(extractflow1)
    min_data_control, q1_data_control, med_data_control, mean_data_control, q3_data_control, max_data_control, var_data_control, total_packets, ack_pkts_sent, pure_acks_sent, actual_data_bytes, pushed_data_pkts, SYN_pkts_sent, FIN_pkts_sent, urgent_data_pkts = extracinformation_2(extractflow1)
...
def wcsv2():
    pro, srcip, dstip, sport, dport = extracinformation(extractflow2)
    min_data_ip, q1_data_ip, med_data_ip, mean_data_ip, q3_data_ip, max_data_ip, var_data_ip, min_data_wire, q1_data_wire, med_data_wire, mean_data_wire, q3_data_wire, max_data_wire, var_data_wire = extracinformation_1(extractflow2)
    min_data_control, q1_data_control, med_data_control, mean_data_control, q3_data_control, max_data_control, var_data_control, total_packets, ack_pkts_sent, pure_acks_sent, actual_data_bytes, pushed_data_pkts, SYN_pkts_sent, FIN_pkts_sent, urgent_data_pkts = extracinformation_2(extractflow2)
    max_win_adv, min_win_adv, zero_win_adv, avg_win_adv, flowlen = extracinformation_3(extractflow2)
    dataframe = pd.DataFrame(
        {'protocol': pro, ' srcip': srcip, ' dstip': dstip, " sport": sport, " dport": dport,
         " min_data_ip": min_data_ip, " q1_data_ip": q1_data_ip,
         " med_data_ip": med_data_ip, "mean_data_ip": mean_data_ip, "q3_data_ip": q3_data_ip,
    ...
    dataframe.to_csv('jingdong.csv', index=False, sep=' ')


wcsv1()
wcsv2()

# extracinformation(flow1)
